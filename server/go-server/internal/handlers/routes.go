...
func (r *Router) SetupRoutes(mux *http.ServeMux) {
	...
	// Team-owner routes: read = Bearer only (registration or session); write = Bearer + X-Session-Token
	requireTeamAuth := RequireTeamAuth(r.teamHandler.GetRepository())
	requireSessionTokenHeader := RequireSessionTokenHeader(r.teamHandler.GetRepository())
	mux.Handle("GET /api/me/team", requireTeamAuth(http.HandlerFunc(r.teamHandler.GetMeTeam)))
	mux.Handle("PUT /api/me/team", requireSessionTokenHeader(http.HandlerFunc(r.teamHandler.UpdateMeTeam)))
	mux.Handle("GET /api/me/players", requireTeamAuth(http.HandlerFunc(r.playerHandler.ListMePlayers)))
	mux.Handle("POST /api/me/players", requireSessionTokenHeader(http.HandlerFunc(r.playerHandler.CreateMePlayer)))
	mux.Handle("GET /api/me/players/{id}", requireTeamAuth(http.HandlerFunc(r.playerHandler.GetMePlayer)))
	mux.Handle("PUT /api/me/players/{id}", requireSessionTokenHeader(http.HandlerFunc(r.playerHandler.UpdateMePlayer)))
	mux.Handle("DELETE /api/me/players/{id}", requireSessionTokenHeader(http.HandlerFunc(r.playerHandler.DeleteMePlayer)))
	mux.Handle("GET /api/me/coaches", requireTeamAuth(http.HandlerFunc(r.coachHandler.ListMeCoaches)))
	mux.Handle("POST /api/me/coaches", requireSessionTokenHeader(http.HandlerFunc(r.coachHandler.CreateMeCoach)))
	mux.Handle("GET /api/me/coaches/{id}", requireTeamAuth(http.HandlerFunc(r.coachHandler.GetMeCoach)))
	mux.Handle("PUT /api/me/coaches/{id}", requireSessionTokenHeader(http.HandlerFunc(r.coachHandler.UpdateMeCoach)))
	mux.Handle("DELETE /api/me/coaches/{id}", requireSessionTokenHeader(http.HandlerFunc(r.coachHandler.DeleteMeCoach)))
	mux.Handle("GET /api/me/allergies", requireTeamAuth(http.HandlerFunc(r.allergyHandler.ListMeAllergies)))
	mux.Handle("POST /api/me/allergies", requireSessionTokenHeader(http.HandlerFunc(r.allergyHandler.CreateMeAllergy)))
	mux.Handle("DELETE /api/me/allergies/{id}", requireSessionTokenHeader(http.HandlerFunc(r.allergyHandler.DeleteMeAllergy)))
