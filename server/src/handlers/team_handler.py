...
def handle_team_get(event):
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute("UPDATE registration_tokens SET last_used_at = NOW() WHERE token = %s", (token,))
            cursor.execute("SELECT nom as nomEquip, email, telefon, sexe, club FROM equips WHERE id = %s", (team_id,))
            ...
            cursor.execute("SELECT nom, cognoms, talla_samarreta as tallaSamarreta FROM jugadors WHERE id_equip = %s", (team_id,))
            jugadors = cursor.fetchall()

            cursor.execute("SELECT nom, cognoms, talla_samarreta as tallaSamarreta, es_principal as esPrincipal FROM entrenadors WHERE id_equip = %s", (team_id,))
