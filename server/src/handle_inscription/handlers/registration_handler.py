    ...
    try:
        ...
        mapped_body = {
            ...
            "jugadors": [
                {
                    "nom": j.get("nom"),
                    "cognoms": j.get("cognoms"),
                    "tallaSamarreta": j.get("tallaSamarreta"),
                }
            ...
            "entrenadors": [
                {
                    "nom": e.get("nom"),
                    "cognoms": e.get("cognoms"),
                    "tallaSamarreta": e.get("tallaSamarreta"),
                    "esPrincipal": e.get("esPrincipal"),
                }
        ...
        missing_fields = [
            field for field in required_fields if not mapped_body.get(field)
        ]
        if missing_fields:
        ...
        with connection.cursor() as cursor:
            ...
            cursor.execute(
                "SELECT id FROM clubs WHERE nom = %s", (mapped_body["club"],)
            )
            ...
            if club:
                ...
                cursor.execute(
                    "INSERT INTO clubs (nom) VALUES (%s)", (mapped_body["club"],)
                )
            ...
            for jugador in mapped_body["jugadors"]:
                cursor.execute(
                    "INSERT INTO jugadors (nom, cognoms, talla_samarreta, id_equip) VALUES (%s, %s, %s, %s)",
                    (
                        jugador["nom"],
                        jugador["cognoms"],
            ...
            for entrenador in mapped_body["entrenadors"]:
                cursor.execute(
                    "INSERT INTO entrenadors (nom, cognoms, talla_samarreta, es_principal, id_equip) VALUES (%s, %s, %s, %s, %s)",
                    (
                        entrenador["nom"],
                        entrenador["cognoms"],
            ...
            for fitxa in mapped_body.get("fitxes", []):
                cursor.execute(
                    "INSERT INTO fitxes_documents (url, id_equip) VALUES (%s, %s)",
                    (fitxa, team_id),
                )
            ...
            cursor.execute(
                "INSERT INTO registration_tokens (team_id, token, expires_at, created_at) VALUES (%s, %s, %s, %s)",
            ...
            cursor.execute(
                "INSERT INTO wa_tokens (team_id, token, expires_at, created_at) VALUES (%s, %s, %s, %s)",
            ...
            registration_url = (
                f"https://{os.getenv('FRONTEND_URL')}/{registration_path}"
            )
