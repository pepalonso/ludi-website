...
export class FitxesJugadorsComponent {
  ...
  @Input() uploadUrl = `${environment.apiBaseUrl}/api/documents/upload`
          ...
          event => {
            if (event.type === HttpEventType.UploadProgress && event.total) {
              ...
              if (event.status === 201) {
                ...
                completedUploads++
                const body = event.body as { id?: number }
                if (body && typeof body.id === 'number') {
                  this.documentIds.push(body.id)
                }
  ...
  nextStep() {
    this.previService.setFormData({ fitxes: this.documentIds })
    this.stepper.next()
  }
